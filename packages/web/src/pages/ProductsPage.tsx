...
const PRICE_OPTIONS = [
  { label: "Tat ca", value: "all", minPrice: undefined, maxPrice: undefined },
  { label: "Duoi 20 trieu", value: "under20", minPrice: undefined, maxPrice: 20_000_000 },
  { label: "20 - 50 trieu", value: "20-50", minPrice: 20_000_000, maxPrice: 50_000_000 },
  { label: "Tren 50 trieu", value: "over50", minPrice: 50_000_000, maxPrice: undefined }
] as const;
  ...
  const displayProducts = useMemo(
    () =>
      sortedProducts.map((item, index) => ({
        ...
        meta: item.reviewCount ? `${item.reviewCount} danh gia` : item.stockQuantity ? `${item.stockQuantity} trong kho` : "San pham moi"
      })),
    [sortedProducts]
  );
      ...
      <div className="mx-auto max-w-7xl px-6 py-16 md:px-12">
        <div className="flex flex-col gap-12 lg:flex-row">
          <aside className="w-full flex-shrink-0 space-y-10 lg:w-72">
            <div>
              <h3 className="mb-6 text-sm font-bold uppercase tracking-wider">Tim kiem</h3>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <input
                  value={searchText}
                  onChange={(event) => setSearchText(event.target.value)}
                  placeholder="Ten xe, mo ta..."
            ...
            <div>
              <h3 className="mb-6 text-sm font-bold uppercase tracking-wider">Dong xe</h3>
              <div className="space-y-3">
                <label className="flex cursor-pointer items-center gap-3">
                  ...
                  />
                  <span className="text-muted-foreground">Tat ca</span>
                </label>
            ...
            <div>
              <h3 className="mb-6 text-sm font-bold uppercase tracking-wider">Phan khuc gia</h3>
              <div className="space-y-3">
          ...
          <main className="flex-grow">
            <div className="mb-10 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <h2 className="text-3xl font-bold">Tat ca san pham</h2>
                <p className="mt-1 text-sm text-muted-foreground">Hien thi {displayProducts.length} mau xe tu API</p>
              </div>
            ...
            {loading ? (
              <div className="py-16 text-center text-muted-foreground">Dang tai san pham...</div>
            ) : error ? (
              <div className="py-16 text-center text-red-500">{error}</div>
            ) : displayProducts.length === 0 ? (
              <div className="py-16 text-center text-muted-foreground">Khong co san pham phu hop voi bo loc API hien tai.</div>
            ) : (
            ...
            <div className="mt-16 flex items-center justify-center gap-2">
              ...
              <button className="h-10 w-10 rounded-lg font-bold transition-colors hover:bg-surface-container-low">2</button>
              <button className="h-10 w-10 rounded-lg font-bold transition-colors hover:bg-surface-container-low">3</button>
              <button className="flex h-10 w-10 items-center justify-center rounded-lg transition-colors hover:bg-surface-container-low">
